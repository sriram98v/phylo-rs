...
impl PhylogeneticDiversity for SimpleRootedTree{
    ...
    {
        let (delta_bar, delta_hat) = self.compute_norm_min();
    ...
    fn compute_norm_min(&self)->(Vec<Vec<(f32, u32)>>,Vec<Vec<(f32, u32)>>)
    {
        ...
        let mut delta_bar = vec![vec![(f32::INFINITY, 0_u32);num_leaves+1];self.nodes.len()];
        let mut delta_hat = vec![vec![(f32::INFINITY, 0_u32);num_leaves+1];self.nodes.len()];
            ...
            if !node.is_leaf(){
                for i in 1..std::cmp::min(num_leaves, self.get_cluster_size(node.get_id()))+1{
                    ...
                    for r in 0..i+1{
                        ...
                        let val_bar = delta_bar[y][r].0 + (self.get_node(y).unwrap().get_weight().unwrap()*(std::cmp::min(r,1) as f32)) 
                            + delta_bar[x][l].0 + (self.get_node(x).unwrap().get_weight().unwrap()*(std::cmp::min(l,1) as f32));
                        let mut e = 0_u32;
                        if l==0{
                            e += delta_bar[y][r].1 + 1;
                        }
        ...
        return (delta_bar, delta_hat);
    }

    fn get_minPD(&self, num_taxa: usize)-><<Self as RootedTree>::Node as RootedWeightedNode>::Weight
    {
        
        self.precomputed_min[self.get_root_id()][std::cmp::min(num_taxa, self.num_taxa())].0
    }
    ...
    {    
        self.precomputed_min[node_id][std::cmp::min(num_taxa, self.num_taxa())].0
    }


    fn get_norm_minPD(&self, num_taxa: usize)-><<Self as RootedTree>::Node as RootedWeightedNode>::Weight
    {
        self.precomputed_norm_min[self.get_root_id()][std::cmp::min(num_taxa, self.num_taxa())].0
    }

    fn backtrack_min(&self, node_id: <<Self as RootedTree>::Node as RootedTreeNode>::NodeID, num_taxa: usize, taxa_set: &mut Vec<<<Self as RootedTree>::Node as RootedTreeNode>::NodeID>)
    {
        if self.is_node_leaf(node_id){
            taxa_set.push(node_id);
        }
        else{
            ...
            let left_edge_weight = self.get_edge_weight(node_id, left_child_id).unwrap();
            let right_edge_weight = self.get_edge_weight(node_id, right_child_id).unwrap();
            if self.get_minPD_node(node_id, num_taxa)==self.get_minPD_node(left_child_id, num_taxa)+left_edge_weight
            {
                if self.is_node_leaf(left_child_id)
                {
                    taxa_set.push(left_child_id);
                }
                else
                {
                    self.backtrack_min(left_child_id, num_taxa, taxa_set)
                }
            }
            else if self.get_minPD_node(node_id, num_taxa)==self.get_minPD_node(right_child_id, num_taxa)+right_edge_weight
            {
                if self.is_node_leaf(right_child_id)
                {
                    taxa_set.push(right_child_id);
                }
                else
                {
                    self.backtrack_min(right_child_id, num_taxa, taxa_set)
                }
                ...
                let ((left_pd, left_edges),(right_pd, right_edges)) = (1..num_taxa).map(|l| (l, num_taxa-l))
                    .map(|(l,r)| (self.precomputed_min[left_child_id][std::cmp::min(l, self.num_taxa())], self.precomputed_min[right_child_id][std::cmp::min(r, self.num_taxa())]))
                ...
                if num_left_taxa>0
                {
                    self.backtrack_min(left_child_id, num_left_taxa, taxa_set);
                }
                if num_right_taxa>0
                {
                    self.backtrack_min(right_child_id, num_right_taxa, taxa_set);
                }
            }    
        }
    }

    // fn get_minPD_node_num_taxa(&self, node_id: <<Self as RootedTree>::Node as RootedTreeNode>::NodeID, num_taxa: usize)->usize
    // {
    //     let num_edges = self.precomputed_min[node_id][std::cmp::min(num_taxa, self.num_taxa())].1;
    //     (num_edges as f32/2_f32) as usize
    // }
