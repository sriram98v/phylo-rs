...
use phylo::tree::stats::PhylogeneticDiversity;
...
#[test]
fn pd() {
    let mut tree = SimpleRootedTree::yule(100).unwrap();
    ...
    (1..101).for_each(|x| {assert_ne!(tree.get_minPD(x), f32::INFINITY);});
    let input_str: String = String::from("(0:1,(1:2,(2:3,(3:4,4:5):6):7):8);");
    let mut tree = SimpleRootedTree::from_newick(input_str.as_bytes());
    tree.precompute_minPDs();
    assert_eq!(tree.get_minPD(2), 11_f32);
    assert_eq!(tree.get_minPD(3), 21_f32);
